...
async function yt(url, quality, type, bitrate, server = 'pt274') {
...
module.exports = {
  ...
  yta(url, resol = '320kbps', server = 'tr225') { return yt(url, resol, 'mp3', resol.endsWith('kbps') ? resol.replace(/kbps/g, '') : resol, server) },
 
  ytv(url, resol = '360p', server = 'vi278') { return yt(url, resol, 'mp4', resol.endsWith('p') ? resol.replace(/p/g, '') : resol, server) },
  servers: ['vi280', '', 'hi231', 'ph232', 'it232']
}
