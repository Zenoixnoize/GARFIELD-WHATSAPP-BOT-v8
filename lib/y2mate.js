...
async function yt(url, quality, type, bitrate, server = 'en56a4') {
  ...
  let res2 = await post(`https://www.y2mate.com/mates/en320/convert`, {
    type: 'youtube',
    _id: id[1],
    v_id: ytId[1],
    ajax: '1',
...
module.exports = {
  ...
  yta(url, resol = '320kbps', server = 'en56a4') { return yt(url, resol, 'mp3', resol.endsWith('kbps') ? resol.replace(/kbps/g, '') : resol, server) },
  
  ytv(url, resol = '360p', server = 'en56a4') { return yt(url, resol, 'mp4', resol.endsWith('p') ? resol.replace(/p/g, '') : resol, server) },
  servers: ['en320']
}
