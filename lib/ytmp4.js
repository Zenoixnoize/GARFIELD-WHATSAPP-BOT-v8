...
async function yt(url, quality, type, bitrate, server = 'en334') {
  let ytId = ytIdRegex.exec(url)
  url = 'https://youtu.be/' + ytId[1]
  let res = await post(`https://www.y2mate.com/mates/en334/analyze/ajax`, {
    url,
    q_auto: 0,
    ajax: 1
  })
...
module.exports = {
  ...
  yta(url, resol = '320kbps', server = 'en334') { return yt(url, resol, 'mp3', resol.endsWith('kbps') ? resol.replace(/kbps/g, '') : resol, server) },
  servers:  ['en334','en290'],
  servers: ['en334','en290'],
  tharinduyt(url, resol = '360p', server = 'en334') { return yt(url, resol, 'mp4', resol.endsWith('p') ? resol.replace(/p/g, '') : resol, server) },
  servers: ['en334','en290']
}
